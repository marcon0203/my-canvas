...
| `agent` | `script` | 写剧本。前一场的结尾送进提示词；`body_of()` 拼结构标记 |
| `agent` | `assets` | 提取资产。分组不合法的整条丢掉；aid 不让模型碰 |
| `agent` | `shots` | 拆镜头。`reconcile()` 核对场次键与景别，并报出丢了几条 |
...
主干 7 步里，**内容那 5 步现在都走真模型**（出视频那步要厂商适配，成片那步是
本机 ffmpeg）：

| 任务 | 模块 | 这条链路的意义在哪 |
|---|---|---|
| 起草大纲 | `outline` | 场次编号由 Rust 补，不信模型编的 id |
| 写剧本 | `script` | **前一场的结尾送进提示词** —— 一场之所以接得上，是因为它知道上一场停在哪儿 |
| 提取资产 | `assets` | 正文真送进去，所以认得出「他/老李/李明」是一个人，也写得出出图用得上的外形描述 |
| 拆镜头 | `shots` | **不再一律三镜** —— 拆几镜看画面变了几次 |
| 补提示词 | `shotprompt` | 按各自景别与引用合成 |
| 延展走向 | `expand` | 前后场次与定稿角色都送，否则和固定句式没差别 |

六条共用一套骨架：`structured::extract`（两条路 + 换路判定 + 真流式）、
`run::Run<I>`（解析 Agent、取密钥、拼 preamble、发事件）、一个 `RunEvent` 变体。
**判别字段两两不同** —— 合成一个 untagged 字段的话，前端每加一条链路就多一处
「靠有没有某个字段来猜」。

每条都遵一条同样的规矩：**编号与结构标记由程序补，不让模型碰**。模型编 id
会重复、会跳号、会和已有的撞，而撞号的错在界面上看起来像正常内容。
所以 `outline::number`、`script::body_of`、`assets` 的 aid（前端编）、
`shots::reconcile` 各自都是纯函数，各自都有测试。

`shots::reconcile` 还多做一件事：**把丢掉的条数带回去**。模型编的场次键、
不认识的景别都整条丢掉，而「20 镜里收了 17 镜」必须在产物卡上说出来 ——
不说的话它看起来像模型只给了 17 镜，而那是两件事（前者说明模型没照约束来，
后者说明提示词要改）。
