...
- **阶段 7 · 配置体系**（已完成）：厂商/模型注册表（火山、DeepSeek、智谱、百炼、
  混元、月之暗面 + 自定义 OpenAI 兼容端点），每个 Agent 单独配**系统提示词**、
...
**「专长」不是文案，是 `owns`** —— 这位 Agent 能接哪些**任务**（`IntentKind`）。
出厂时每个任务恰好一位认领（有测试兜底：没有孤儿，也没有两人抢）。
`owns` 只是**出厂默认**：运行时谁接哪个任务由设置里的配置说话，
技能卡也按配置派生，所以卡片和能力不会走偏。改分工在
**设置 → 智能体管理 → 任务分工**：12 个任务一行一个，能看出谁负责、
结果写到哪、做法是写在 Skill 文件里还是内置在程序里。

> **「任务」和「Skill」不是一回事。** 任务 = 用户能提的一件需求（`domain/agent/tasks.ts`
> 的注册表）；Skill = 磁盘上一份写给模型的操作说明文件（`resources/skills/`、
> `skill/` crate）。一个任务的做法**可以**写在 Skill 文件里（那种可以改），
> 也可以内置在程序里。这两个词以前在代码和界面里都叫 Skill，撞在一起，
> 「那张表到底在说什么」谁都看不懂 —— 现在分开了。

**这五位是内置的，删不掉**（各自对应一个环节，删了那个环节就没人当班），
但**可以自己建新的**：设置 → 智能体管理 → 新建智能体，走一个五步向导
（基本信息 → 系统提示词 → 任务与工具 → 模型 → 执行权限），也可以复制现有的一位。
自己建的那几位不占环节，只按配置接活儿。
...
domain/agent/   纯逻辑：roster 班底 / tasks 任务注册表（12 件事）/ config 每位的配置与校验 /
                tools 工具注册表 / router 自由文本路由 / drafts 草稿生成 / plans 计划装配
...
| 所有环节共用一个万能 Agent | 内置五位各有专长（`roster.ts`），接不了的活儿转交给对的那位；不够用时自己建 |
